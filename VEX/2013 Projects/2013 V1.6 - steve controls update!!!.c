...
void AutoPlaceholder1() //red
{
	SensorValue(I2C_1) = 0; //reset sensor
	SensorValue(I2C_2) = 0;
  Drive(127,60,1500); //go forward
  Drive(-127,0,1500); //go back
  Drive(0,-60,900); //move arm down
  Turn(127,500); //turn right
  Drive(127,0,1500); //go forward
  Turn(-127,500); //turn left
  Drive(127,60,1500); //go forward
  Drive(-127,0,1500); //go back
  Drive(0,-60,900); //move arm down
}

void AutoPlaceholder2() //blue
{
	SensorValue(I2C_1) = 0; //reset sensor
	SensorValue(I2C_2) = 0;
  Drive(127,60,1500); //go forward
...
task autonomous()
{
  ...
  if (SensorValue(RedOrBluSwitch) > 1)
  {
  	...
  	{
  		AutoPlaceholder1(); //CODE FOR RED AND RIGHT GOES HERE
  	}
  }
  ...
  else if (SensorValue(RedOrBluSwitch) == 0)
  {
  	...
  	{
  		AutoPlaceholder2(); //CODE FOR BLU AND RIGHT GOES HERE
  	}
  }
}
 	  ...
 	  else if (SensorValue(controlSwitch) == 0) //if theres a switch then steve controls (joystick magic!!!)
 		{
 			while (1==1)
		 	  ...
		 	  if (abs(vexRT[Ch4]) > abs(vexRT[Ch3])) //if absolute value of LR > absolute value of UD
				{
				if ((-vexRT[Ch4]) > (vexRT[Ch4])) { //joystick is left, turn left
					motor[BRmotor] = -(vexRT[Ch4]); //LR is the chosen control
					motor[FRmotor] = -(vexRT[Ch4]); //right side is inverted speed of LR
					motor[BLmotor] = 0;
					motor[FLmotor] = 0;
				} else { //assuming that joystick is right or 0
					motor[BRmotor] = 0; //LR is the chosen control
					motor[FRmotor] = 0; //left side controlled by LR
