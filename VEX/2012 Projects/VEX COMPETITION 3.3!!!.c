...
#pragma config(Motor,  port8,           shoulderMotor, tmotorVex393, openLoop, reversed)
...
Helper Right U and D buttons = shoulder
Helper 8R button = claw
...
void ClawStuff(int x) //this code in a nutshell (hold 8R to open)
{
	if(vexRT[Btn8RXmtr2] == 1) //if helper control button 8R is hit
	{
		motor[clawMotor] = x; //claw motor opens
	}
	else if(vexRT[Btn8RXmtr2] == 0) //else if helper control button 8R isn't hit
	{
...
{
	if(vexRT[Btn7LXmtr2] == 1) //if control button 7L is hit
	{
		motor[turnTableMotor] = x; //spin left
	}
	else if(vexRT[Btn7RXmtr2] == 1) //else if helper control button 7R is hit
	{
