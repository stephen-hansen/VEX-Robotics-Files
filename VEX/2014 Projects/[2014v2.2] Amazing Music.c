...
task autonomous()
{
    ...
    playSoundFile("MEGAMAN.wav");
		...
		playSoundFile("DARUDE.wav");
		displayNextLCDString("song name?");
		displayLCDPos(1,0);
		displayNextLCDString("DUDUDUDUDUDUDUDU");
		} else if (message == 6) {
		playSoundFile("SANIC.wav");
		displayNextLCDString("you're too slow");
		displayLCDPos(1,0);
		displayNextLCDString("gotta go fast");
		} else if (message == 7) {
		playSoundFile("RICKROLL.wav");
		displayNextLCDString("u know the rules");
		displayLCDPos(1,0);
		displayNextLCDString("and so do i");
	}
	if (auto == 1) { // left
		simpleAuto();
		}	else if (auto == 2) { // center
