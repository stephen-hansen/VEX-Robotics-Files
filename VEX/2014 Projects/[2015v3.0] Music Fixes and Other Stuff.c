...
void debugControls() {
	...
	// SOUND EFFECT BUTTONS are mapped to all shoulder buttons (currently all are set to airhorn)
	if ((vexRT[Btn5U]) == 1) {
		startTask(airhorn);
	}
	if ((vexRT[Btn5D]) == 1) {
	...
	while (true)
	{
		...
		else if (SensorValue(controlSwitch) == 0) //if theres a switch then enable experimental motion controls
		{
			while(true)
			{
				...
				motor[LC]=(vexRT[AccelYXmtr2]); //partner controls conveyor
				motor[RC]=(vexRT[AccelYXmtr2]);
			}
		}
	}
}
