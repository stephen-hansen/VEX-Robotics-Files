...
void autoturn() {
	...
	while (SensorValue[gyro] > 0 || SensorValue[gyro] < 0) {
...
void autoturnAngle(float turn) {
		turn = turn + 1800;
		while (SensorValue[gyro] > (10 + turn) || SensorValue[gyro] < (-10 + turn)) {
		if ((SensorValue[gyro] > (10 + turn) && SensorValue[gyro] <= (1800 + turn)) || (SensorValue[gyro] < (-1800 + turn))) {
			motor[FR] = 127;
			motor[BR] = 127;
			motor[FL] = -127;
			motor[BL] = -127;
		}	else if ((SensorValue[gyro] < (-10 + turn) && SensorValue[gyro] >= (-1800 + turn)) || (SensorValue[gyro] > (1800 + turn))) {
			motor[FR] = -127;
			motor[BR] = -127;
			motor[FL] = 127;
			motor[BL] = 127;
		}
		while (SensorValue[gyro] > turn + 1 || SensorValue[gyro] < turn - 1) {
			if (SensorValue[gyro] > turn + 1) {
				motor[FR] = 30;
				motor[BR] = 30;
				motor[FL] = -30;
				motor[BL] = -30;
			}	else if (SensorValue[gyro] < turn - 1) {
				motor[FR] = -30;
				motor[BR] = -30;
				motor[FL] = 30;
				motor[BL] = 30;
...
void autoturnToGoal() {
	if (COLOR == 0) { //blue
		...
		SHOOTSPEED = floor((UNMODIFIEDSPEED * 60 * (1/(2*PI*RADIUS)) * (127/120)));
