...
int COLOR;
int SHOOTSPEED = 127;
int NETSPEED = 110; //Skills speed
int COOLSPEED = 100; //Cooldown speed
...
void shoot() {
	if (SensorValue[skills] == 0) {
		if (SensorValue[detector] == 1) {
			...
			motor[shooter2] = NETSPEED;
			motor[shooter3] = NETSPEED;
			motor[shooter4] = NETSPEED;
			}	else {
			if (motor[shooter1] > (COOLSPEED)) {
				...
				motor[shooter2] = COOLSPEED;
				motor[shooter3] = COOLSPEED;
				motor[shooter4] = COOLSPEED;
			}
		}
  } else {
		if (SensorValue[detector] == 1) {
			...
			if (motor[shooter1] > (SHOOTSPEED - 10)) {
				...
				motor[shooter2] = SHOOTSPEED - 10;
				motor[shooter3] = SHOOTSPEED - 10;
				motor[shooter4] = SHOOTSPEED - 10;
			}
		}
	}
}
...
task usercontrol() {
	...
	while (true) {
		...
		if (vexRT[Btn7D] == 1 && soundPlay == false) {
			soundPlay = true;
			startTask(ffvii_boss);
		}
		if (vexRT[Btn8D] == 1) {
