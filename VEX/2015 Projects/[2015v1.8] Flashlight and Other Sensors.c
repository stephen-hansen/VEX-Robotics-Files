...
task flashlightCtrl () { //Control flashlight
	while (true) {
		if (vexRT[Ch3] > 10 && flashValue < 127) {
			flashValue = flashValue + 1;
			motor[flashlight] = flashValue;
		}	else if (vexRT[Ch3] < -10 && flashValue > -1) {
			flashValue = flashValue - 1;
			motor[flashlight] = flashValue;
		}
		wait1Msec(50);
...
task usercontrol() {
	...
	while (true) {
		...
		motor[lift1] = vexRT[Ch1];
		motor[lift2] = vexRT[Ch1];
		motor[lift3] = vexRT[Ch1];
		motor[lift4] = vexRT[Ch1];
		motor[lift5] = vexRT[Ch1];
		motor[lift6] = vexRT[Ch1];
  	...
  	else if (vexRT[Btn7D] == 1) { //Activate lift only at end
  		stopTask(nodebug);
  		stopTask(buttonDebug);
  		stopTask(sensorDebug);
  		stopTask(otherDebug);
