...
task usercontrol()
{
	...
	while (true)
	{
		...
		if ((vexRT[Btn5U] == 0 && vexRT[Btn6D] == 0) && SensorValue[limiter] <= lockLim && limit == 0 && locked == false) {
			stopTask(unlockIt);
			startTask(lockIt);
	  } else if ((vexRT[Btn5U] == 1 || vexRT[Btn6D] == 1) && SensorValue[limiter] <= lockLim && limit == 0 && locked == true) {
			stopTask(lockIt);
	  	startTask(unlockIt);
		} else if (vexRT[Btn5U] == 1 && limit == 0 && locked == false) {
			lifting(127);
			mode2 = 1;
		}	else if ((vexRT[Btn6D] == 1 || vexRT[Btn6U] == 1) && limit == 0 && locked == false) {
			lifting(-127);
			mode2 = 2;
			} else {
			lifting(0);
