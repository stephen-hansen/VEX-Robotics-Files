...
void driveSquares(int speed, float numSquares) {
	...
	while (sgn(SensorValue[quadEnc] - (startQuad + numSquares*driveEncVal)) == quadSign && quadSign != 0 && time1[T2] <= 2000) {
	}
...
void turnDegreesGyro(int speed, int degrees) {
	...
	while (sgn(SensorValue[gyro] - (startDegree + degrees*10)) == degreeSign && (degreeSign != 0) && time1[T2] <= 2000) {
	}
	writeDebugStreamLine("%4f", time1[T2]);
	motor[BL] = 0;
	motor[FL] = 0;
...
void liftAngle(int speed, float angle) { //Set the lift to a specified angle
	...
	while ((sgn(SensorValue[liftEnc] - (liftEncVal*(angle/135) + minLift)) == angleSign) && (angleSign != 0) && time1[T2] <= 2000) { //is 135 deg a good estimate for entire lift?
	}
	writeDebugStreamLine("%4f", time1[T2]);
	motor[liftML] = 0;
	motor[liftMR] = 0;
...
void clawAngle(int speed, float angle) { //Set the claw to a specified angle
	...
	while ((sgn(SensorValue[clawEnc] - (clawEncVal*(angle/180) + minClaw)) == clawSign) && (clawSign != 0) && time1[T2] <= 2000) { //is 135 deg a good estimate for entire lift?
	}
	writeDebugStreamLine("%4f", time1[T2]);
	motor[clawL] = 0;
	motor[clawR] = 0;
...
void driveLiftAndClaw(float dSpeed, float lSpeed, float cSpeed, float dSquares, float lAngle, float cAngle) {
	clearTimer(T2);
	dSquares = -dSquares;
	startQuad = SensorValue[quadEnc];
    ...
    || ((sgn(SensorValue[clawEnc] - (clawEncVal*(cAngle/180) + minClaw)) == clawSign) && (clawSign != 0))) && time1[T2] <= 2000) {
...
void auto1() { //Go to corner star, throw, knock off side stars
	...
	motor[clawL] = -127;
	motor[clawR] = -127;
	driveSquares(127, -1.75); //Go up to fence
	//liftAngle(liftSpeed, 135); //Lift up
	motor[clawL] = 0;
	motor[clawR] = 0;
	//clawAngle(clawSpeed, 90); //Open up claw
	//TEST
	driveLiftAndClaw(0, liftSpeed, clawSpeed*0.75, 0, 135, 170);
	//TEST
	liftAngle(liftSpeed, 0); //Lift down
	driveSquares(127, 1.5); //Go up near back
	...
	motor[clawR] = 0;
	//TEST
	driveLiftAndClaw(0, liftSpeed, clawSpeed*0.75, 0, 135, 135);
	//TEST
...
void auto2() { //Go to middle stars, throw, knock off side stars
	...
	driveSquares(127, 1); //Drive forward a square
	clawAngle(clawSpeed, 180); //Grab the stars
	motor[clawL] = -127;
	motor[clawR] = -127;
	driveSquares(127, -1.75); //Go back
	turnDegreesGyro(turnSpeed, 30*(-turnDir)); //Align backwards; go right/left if left/right
	driveSquares(127, -1.25); //Go backwards to wall
	//liftAngle(liftSpeed, 135); //Lift up, knock off fence
	motor[clawL] = 0;
	motor[clawR] = 0;
	//TEST
	driveLiftAndClaw(0, liftSpeed, clawSpeed*0.5, 0, 135, 135);
	//TEST
	//clawAngle(clawSpeed, 90); //Open up claw wide, let go of stars
}

void auto3() { //Go to middle cube, throw, knock off middle stars
	driveSquares(127, -0.5); //Move up one
	clawAngle(clawSpeed, 180); //Close claw
	motor[clawL] = -127;
	motor[clawR] = -127;
	...
	//clawAngle(clawSpeed, 90); //Open up claw
	liftAngle(liftSpeed, 0); //Lift down
	clawAngle(clawSpeed, 0); //Open up
	driveSquares(127, 0.325);
	turnDegreesGyro(turnSpeed, 90*turnDir); //Go left/right if left/right
	driveSquares(127, 1.5); //Go up to cube
	turnDegreesGyro(turnSpeed, 90*(-turnDir)); //Go right/left if left/right
	driveSquares(127, 0.5); //Go to cube
	clawAngle(clawSpeed, 165); //Grab cube
	motor[clawL] = -127;
	motor[clawR] = -127;
	//liftAngle(127, 30); //Lift up a bit
	driveSquares(127, -1); //Go back
	//liftAngle(127, 60); //Lift up a bit
	liftAngle(127, 135); //Lift up
	//TEST
	//driveLiftAndClaw(0, 127, clawSpeed*0.5, 0, 135, 135);
	//TEST
	motor[clawL] = 0;
	motor[clawR] = 0;
...
void auto4() { //Clear the field
	...
	clawAngle(clawSpeed, 0); //Open up
	driveSquares(127, 0.325);
	turnDegreesGyro(100, 75*turnDir); //Go left/right if left/right
	driveSquares(127, 1.5); //Go up to cube
	turnDegreesGyro(100, 75*(-turnDir)); //Go right/left if left/right
	driveSquares(127, 0.5); //Go to cube
	clawAngle(clawSpeed, 165); //Grab cube
	motor[clawL] = -127;
	motor[clawR] = -127;
	...
	motor[clawR] = 0;
	//TEST
	driveLiftAndClaw(0, liftSpeed, clawSpeed*0.25, 0, 135, 135);
	//TEST
...
void pre_auton()
{
	...
	startTask(timer);
	while (ready == false) {
		if (time1[T1] > 1500000) { //Failsafe, normally 3000
			ready = true;
		}
...
task autonomous()
{
	...
	if (automode == 1) { //Side stars
		...
		auto2();
		} else if (automode == 3) { //Middle cube
		auto3();
		} else if (automode == 4) { //CLEAR FIELD
		auto4();
		} else {
		auto1();
	}
