...
#pragma config(Sensor, in8,    speedReader,    sensorPotentiometer)
...
void getSpeed() {
	speed = ((SensorValue[speedReader] - initPotent)/time)*(265/4095)
}
...
void pre_auton()
{
  ...
  if (SensorValue[betaTest] == 0) {
	...
	SensorType[speedReader] = sensorNone;
	wait1Msec(1000);
	SensorType[speedReader] = sensorPotentiometer;
}
  	...
  	debug = vexRT[Ch3];
		...
		if (nVexRCReceiveState & vrXmit2) { //Enable partner controls
			...
			if (vexRT[Btn5U] == 1) {
				if (pFlag == false) {
					pFlag = true;
					initPotent = SensorValue[speedReader];
					startTask(timer);
				}
				motor[LiftTL] = 127;
				motor[LiftBL] = 127;
