...
#pragma config(Motor,  port5,           BR,            tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_1)
...
task usercontrol()
{
  ...
  while (true)
  {
	  ...
	  if (vexRT[Btn8D] == 0 && vexRT[Btn8U] == 0 && vexRT[Btn5D] == 0 && vexRT[Btn6D] == 0) {
		...
		//Auto generator
		if ((oldLeftSpeed != motor[BL]) || (oldRightSpeed != motor[BR]) || (oldLiftSpeed != motor[LiftBL])) {
			writeDebugStreamLine("while (time < %d) {", time);
