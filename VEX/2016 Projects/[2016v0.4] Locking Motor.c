...
#pragma config(Motor,  port1,           lock,          tmotorVex393_HBridge, openLoop)
...
task usercontrol()
{
	...
	while (true)
	{
		...
		if (vexRT[Btn6U] == 1 && nMotorEncoder[LiftBR] <= -50 && locked == "false" && limit == 0) {
			startTask(lockIt);
	  } else if (vexRT[Btn5U] == 1 && nMotorEncoder[LiftBR] <= 0 && locked == "true" && limit == 0) {
			startTask(unlockIt);
