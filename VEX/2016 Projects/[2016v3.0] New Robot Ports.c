...
int clawEncVal = 400; //For one full grab (90 deg)
int minClaw = 0;
...
//DRIVING - COMPLETE
//TURNING - COMPLETE
//LIFTING - NEED TO TEST AND VERIFY
//CLAWING
...
task encoderValues() {
	while (true) {
		...
		sprintf(linee1, "%4d", SensorValue[liftEnc]);
...
void driveSquares(int speed, int numSquares) {
...
//turnDegreesGyro: rotates the wheels in quickest direction towards specified degree plane. is not incremental;
//degrees specified are exact location and do not change throughout program. compass-type rotation; 0 deg is initial pos.
int degreeSign;
void turnDegreesGyro(int speed, int degrees) {
	degrees = -degrees;
	degreeSign = sgn(SensorValue[gyro] - degrees*10);
	motor[BL] = speed*(degreeSign);
	motor[FL] = speed*(degreeSign);
	motor[BR] = speed*(-degreeSign);
	motor[FR] = speed*(-degreeSign);
	while (sgn(SensorValue[gyro] - degrees*10) == degreeSign && (degreeSign != 0)) {
...
int angleSign;
void liftAngle(int speed, int angle) { //Set the lift to a specified angle
...
int clawSign;
void clawAngle(int speed, int angle) { //Set the claw to a specified angle
...
void auto1() { //Go to corner star, throw, knock off side stars
	...
	driveSquares(127, 0.5); //Drive forward half a square
	clawAngle(clawSpeed, 180); //Grab the star
	turnDegreesGyro(turnSpeed,180*turnDir); //Align backwards
	driveSquares(127, -2); //Go backwards to wall
...
void auto2() { //Go to middle stars, throw, knock off side stars
	...
	driveSquares(127, 1.5); //Drive forward 1.5 squares in order to not hit partner
	clawAngle(clawSpeed, 180); //Grab the stars
	driveSquares(127, -2); //Go back to starting tile (a little past to align with fence)
	turnDegreesGyro(turnSpeed, 180*(-turnDir)); //Align backwards
	driveSquares(127, -2); //Go backwards to wall
	liftAngle(liftSpeed, 135); //Lift up, knock off fence
	clawAngle(clawSpeed, 90); //Open up claw wide, let go of stars
...
void auto3() { //Go to middle cube, throw, knock off side stars
	clawAngle(clawSpeed, 90); //Open up claw
	driveSquares(127, 1); //Move forward
	turnDegreesGyro(turnSpeed, 90*(-turnDir)); //Go right/left if left/right
	driveSquares(127, 1.5); //Move forward to grab cube
	clawAngle(clawSpeed, 180); //Grab the cube
	driveSquares(127, -2); //Go backwards to align with fence
	turnDegreesGyro(turnSpeed,180*(-turnDir)); //Align backwards
	driveSquares(127, -1); //Go backwards to wall
	liftAngle(liftSpeed, 135); //Lift up, knock off fence
	clawAngle(clawSpeed, 90); //Open up claw wide, let go of cube
}
...
void pre_auton()
{
	...
	startTask(timer);
	while (ready == false) {
		if (time2 > 3000) { //Failsafe, normally 3000
			ready = true;
		}
