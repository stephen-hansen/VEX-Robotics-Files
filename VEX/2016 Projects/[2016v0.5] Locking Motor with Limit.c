...
#pragma config(Motor,  port1,           lock,          tmotorVex393_HBridge, openLoop, reversed)
...
task usercontrol()
{
	...
	while (true)
	{
		...
		if (vexRT[Btn6U] == 1 && SensorValue[limiter] <= lockLim && locked == "false" && limit == 0) {
			startTask(lockIt);
	  } else if (vexRT[Btn5U] == 1 && SensorValue[limiter] <= lockLim && locked == "true" && limit == 0) {
			startTask(unlockIt);
		} else if (vexRT[Btn5U] == 1 && locked == "false") {
			lifting(127);
			mode2 = 1;
			}	else if (vexRT[Btn6U] == 1 && locked == "false") {
			lifting(-127);
			mode2 = 2;
			} else {
			lifting(0);
			if (mode2 != 0) {
			    if (mode2 == 1) {
			    writeDebugStreamLine("locking(127);");
					writeDebugStreamLine("wait1Msec(lockTime);");
					writeDebugStreamLine("locking(0);");
					writeDebugStreamLine("while (nMotorEncoder[LiftBR] < %d) {", nMotorEncoder[LiftBR]);
					writeDebugStreamLine("	lifting(auSpeed);");
					writeDebugStreamLine("}");
					writeDebugStreamLine("lifting(0);");
					writeDebugStreamLine("wait1Msec(100);");
					}	else if (mode2 == 2) {
					writeDebugStreamLine("while (nMotorEncoder[LiftBR] > %d) {", nMotorEncoder[LiftBR]);
					writeDebugStreamLine("	lifting(-auSpeed);");
